...
class AI_Olympics:
    def __init__(self, random_selection, minimap, game_name="olympic_intergrated", **kwargs):
        ...
        self.running_Gamemap = create_scenario("running-competition")
        self.running_game = Running_competition(self.running_Gamemap, vis = self.vis, vis_clear=self.vis_clear, agent1_color = 'light red', agent2_color='blue')
        ...
        elif game_name == "olympics-running":
            self.game_pool = [{"name": 'running-competition', 'game': self.running_game}]
        elif game_name == "olympics-wrestling":
    ...
    def reset(self):
        ...
        selected_game_idx = self.selected_game_idx_pool[self.current_game_count]
            ...
            # MapStats = create_scenario("map" + str(idx), file_path=os.path.join(current_path, "scenario/running_competition_maps/maps.json"))
            # self.running_game = Running_competition(MapStats, map_id=None, vis = self.vis, vis_clear=self.vis_clear, agent1_color = 'light red', agent2_color='blue')
