...
class PPOAgent(object):
    ...
    def _save_train_history(self):
        ...
        torch.save(self.actor.state_dict(), f"{self.path2save_train_history}/{self.env_name}/{data_time.month}_{data_time.day}_{data_time.hour}_{data_time.minute}/actor.pth")
        torch.save(self.critic.state_dict(), f"{self.path2save_train_history}/{self.env_name}/{data_time.month}_{data_time.day}_{data_time.hour}_{data_time.minute}/critic.pth")
        torch.save(self.encoder.state_dict(), f"{self.path2save_train_history}/{self.env_name}/{data_time.month}_{data_time.day}_{data_time.hour}_{data_time.minute}/encoder.pth")

        pd.DataFrame({"actor loss": self.actor_loss_history, "critic loss": self.critic_loss_history}).to_csv(f"{self.path2save_train_history}/loss_logs.csv")

        pd.DataFrame(data=self.scores, columns=["scores"]).to_csv(f"{self.path2save_train_history}/score_logs.csv")
