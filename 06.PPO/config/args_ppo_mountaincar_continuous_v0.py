...
def get_args(rest_args):
    ...
    parser.add_argument('--env_name', type=str, default='MountainCarContinuous-v0', help='name of environment')
    ...
    parser.add_argument('--rollout_len', type=int, default=1000, help='num t-steps per one rollout')
    parser.add_argument('--total_rollouts', type=int, default=50000, help='num rollouts')
    parser.add_argument('--num_epochs', type=int, default=64,
                        help='num weights updation iteration for one policy update')
    parser.add_argument('--batch_size', type=int, default=1000, help='data batch size for weights updating')

    # agent net
    parser.add_argument('--obs_dim', type=int, default=2, help='dimension od observaion')
    parser.add_argument('--continuous', type=boolean_argument, default=True,
                        help='True of environments with continuous action space')
    parser.add_argument('--act_dim', type=int, default=1, help='dimension of action')

    # agent nets optimizers
    parser.add_argument('--actor_lr', type=float, default=1e-3, help='learning rate for actor optimizer')
    parser.add_argument('--critic_lr', type=float, default=5e-3, help='learning rate for actor optimizer')

    # etc.
    parser.add_argument('--is_evaluate', type=boolean_argument, default=False, help='for evaluation')
    parser.add_argument('--solved_reward', type=int, default=92, help='desired reward')
